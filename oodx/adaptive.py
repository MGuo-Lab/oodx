...
class AdaptiveSampler:
    ...
    def modified_expected_improvement(self, model, y, sense):
        ...
        m.mod_ei_con = pyo.Constraint(
            expr=m.mod_ei
            == pyo.sqrt((constant_value + m.mdl_std.outputs[0]) / (2 * 3.1416))
            * pyo.exp(
                -((y_opt - m.mdl.outputs[0]) ** 2)
                / (2 * (constant_value + m.mdl_std.outputs[0]))
            )
        )
    ...
    def _get_delaunay_centroids_and_sizes(self, x):
        ...
        for i, s in enumerate(self.delaunay.simplices):
            vals = x[s, :]
            centroids[i, :] = [
                sum(vals[:, j]) / vals.shape[0] for j in range(vals.shape[1])
            ]
    ...
    def _delaulay_triangle_milp(self, centroids, sizes):
        ...
        m.obj = pyo.Objective(
            expr=sum(m.y[i] * sizes[i] for i in m.n_dt), sense=pyo.maximize
        )
        return m
