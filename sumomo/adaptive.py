...
class AdaptiveSampler:
    ...
    def max_gp_std(self):
        ...
        '''
        m = pyo.ConcreteModel()
        m.mdl = pyo.Block(rule= 
            BlockFormulation(self.model).rule(return_std=True)
        )
    ...
    def max_constrained_gp_std(self):
        ...
        '''
        m = self.max_gp_std()
        m.feas = pyo.Block(rule= 
            BlockFormulation(self.classifier).rule()
        )
    ...
    def modified_expected_improvement(self, y, sense,):
        ...
        m = pyo.ConcreteModel()
        m.mdl = pyo.Block(
            rule=BlockFormulation(self.model).rule()
        )
        m.mdl_std = pyo.Block(
            rule=BlockFormulation(self.model).rule(return_std=True)
        )
        m.n_inputs = set(range(len(self.space)))
        m.inputs = pyo.Var(m.n_inputs, bounds=self.space)
        m.mod_ei = pyo.Var()
