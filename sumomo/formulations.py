...
class SumoBlock:
    ...
    def get_formulation(self, return_std=False):
        ...
        if self.model.name == 'GPR':
            ...
            if self.model.kernel_name == 'linear':
                if return_std:
    ...
    def _gpr_linear_std_rule(self, m):
        ...
        m.gpr_std = pyo.Constraint(expr=
            m.outputs[0] == - sum(
                constant_value * (sigma_0 ** 2 + sum(m.inputs[j] * x_train[i, j] for j in range(m))) 
                * sum(inv_K[i, k] * constant_value * (sigma_0 ** 2 + sum(m.inputs[j] * x_train[k, j] for j in range(m))) 
    ...
    def _gpr_polynomial_std_rule(self, m):
        ...
        m.gpr_std = pyo.Constraint(expr=
            m.outputs[0] == - sum(
                constant_value * (sigma_0 ** 2 + sum(m.inputs[j] * x_train[i, j] for j in range(m))) ** porder 
                * sum(inv_K[i, k] * constant_value * (sigma_0 ** 2 + sum(m.inputs[j] * x_train[k, j] for j in range(m))) ** porder
                for k in n_samples) for i in n_samples
            )
        )
    ...
    def _nn_relu_rule(self, m):
        ...
        m.inputs = pyo.Var(m.nn.nodes[0])
        m.outputs = pyo.Var(m.nn.nodes[len(self.model.layers) - 1])
        m.y = pyo.Var(set([(i, j) for i in m.nn.nodes for j in m.nn.nodes[i]]), domain=pyo.Binary)
    ...
    def _nn_hardsigmoid_rule(self, m):
        ...
        m.inputs = pyo.Var(m.nn.nodes[0])
        m.outputs = pyo.Var(m.nn.nodes[len(self.model.layers) - 1])
        m.p = pyo.Var(set([(i, j) for i in m.nn.nodes for j in m.nn.nodes[i]]), domain=pyo.Binary)
        m.q = pyo.Var(set([(i, j) for i in m.nn.nodes for j in m.nn.nodes[i]]), domain=pyo.Binary)
